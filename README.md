...
6. Opens in your preferred editor
...
git-wt attempts to open the new worktree in your preferred editor. For security, only the following editors are supported for automatic opening:

`code`, `cursor`, `vim`, `vi`, `nano`, `emacs`, `nvim`, `subl`, `mate`, `atom`

It resolves the editor in the following priority order:

1. `--editor` flag override
2. `git config worktree.editor`
3. `VISUAL` environment variable
4. `EDITOR` environment variable
5. Fallback: `code` (VS Code)
6. Fallback: `cursor` (Cursor IDE)

If the resolved editor is not in the allowlist or is not found, git-wt prints `cd` instructions instead.
